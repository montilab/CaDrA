...
#' 
#' @param mat a matrix of binary features (required). \code{NOTE:} 
...
#' @param input_score a vector of continuous values for a response of interest 
...
#' customized_rowscore <- function(mat, input_score, alternative){
...
)
{
  ...
  # If mat has only one column, it must be converted to a row-wise 
  # matrix form as it is needed for backward_forward_search() computation
  ...
  if(is.null(rownames(mat)))
    stop("The mat object does not have rownames or featureData ",
         "to track the features by. Provide unique features or rownames ",
  ...
  if(length(input_score) != ncol(mat)){
    stop("The input_score variable must have the same length ",
         "as the number of columns in mat.\n")
  }else{
    ...
    # match colnames of expression matrix with names of provided 
    # input_score values
  ...
  if(any(rowSums(mat) == 0) || any(rowSums(mat) == ncol(mat))){
    warning("Provided matrix has some features that are either all 0 or 1. ",
  ...
  if(!"mat" %in% custom_args){
    stop("custom_function() must contain 'mat' as ",
  ...
  # using the required mat argument as parameter. removing any mat variables 
  # from the custom_parameter list since it is redundant. 
  if("mat" %in% names(custom_parameters)){
    warning("Removing 'mat' from the custom_parameter list. ",
            "Using the required 'mat' as argument instead.")
  ...
  if("input_score" %in% names(custom_parameters)){
    warning("Removing 'input_score' from the custom_parameter list. ",
            "Using the required 'input_score' as argument instead.")
  ...
  ## combine a mat and input_score variables with additional parameters 
