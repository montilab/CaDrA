...
)
{
  ...
  if ( do_pval && !is.null(weights) ) 
    warning("p-value is meaningless w/ weighted score")
  if ( !is.null(weights) && length(weights) != n.x ) 
    stop("weights must be same length as ranked list: ", 
         length(weights), " vs ", n.x)
  x.axis <- y.axis <- NULL
  ...
  if ( !is.null(weights) )
  {
    ...
    Pmis <- cumsum(Pmis); Pmis <- Pmis/(n.x-n.y)
    Phit <- rep(0, n.x); Phit[y] <- weights; 
    Phit <- cumsum(Phit); Phit <- Phit/Phit[n.x]
    z <- Phit-Pmis
  ...
  else
  {
    ...
    #
    Y <- sort(c(y-1,y)); Y <- Y[diff(Y)!=0]; 
    y.match <- match(y,Y); #if ( any(is.na(y.match)) ) browser()
