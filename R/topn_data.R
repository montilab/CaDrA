...
#'
#' A list of lists returned from topn_eval() using simulated dataset 
#' \code{ES = sim.ES}, \code{input_score = ncol(sim.ES):1}, \code{top_N = 7}, 
...
#' Over top_N = 7 feature searches, a list of ESet, along with its 
...
#' containing ESet, corresponding best scores, and input_score of the 
#' top_N feature searches.
...
#' data(topn.list)
#' 
#' # To fetch each search result 
#' # (Expression Set and score of the corresponding meta-feature)
#' # For N=1 (result when the search is initiated with the 
#' # top-scoring starting feature)
#' topn.list[[1]]
#' 
#' # To fetch just the ExpressionSet object
#' topn.list[[1]]$eset
#' 
#' # ExpressionSet for the search result when starting with 
...
#' meta_plot(best_meta)
#' 
#' # Or you can visualize the overlap of features across the top N (N=7) 
#' # returned meta-features
