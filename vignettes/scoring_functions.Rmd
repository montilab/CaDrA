...
In **CaDrA** package, there are four scoring functions that were implemented to search for relevant subsets of genomic features that are likely associated with a specific outcome of interest (i.e. protein expression, pathway activity, etc.)
...
- Both `candidate_search()` and `topn_eval()` are basically the same function as we previously combined functionality of `topn_eval()` into `candidate_search()` as one unified function

Below, we are evaluating the candidate search over top 7 starting seed features using four different scoring methods mentioned above.
...
1. A `binary features matrix` also known as `Feature Set` (such as somatic mutations, copy number alterations, chromosomal translocations, etc.) The 1/0 vectors indicating the presence/absence of ‘omics’ features in the samples. The `Feature Set` must be an object of class **ExpressionSet** from **Biobase** package) 
